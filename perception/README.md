...
- `CLASS_RADIUS_M` in `m1_perception.py` has only `"bottle"` from the spec;
  other classes are unverified placeholders -- measure and correct.
- If target objects aren't COCO classes, fine-tune the detector on labeled
  photos of them (see `../tools/video_to_dataset.py`).
