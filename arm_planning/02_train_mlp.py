...
def main():
    ...
    parser.add_argument("--resume-path", type=str, default="ik_model_resume.pt",
                         help="Full training-state file, saved after every epoch. If it "
                              "already exists, training automatically continues from it "
                              "instead of starting over -- this is what survives a crash, "
                              "sleep, or forced shutdown mid-run.")
    ...
    total_epochs = args.epochs  # length of the cosine schedule -- see note below

    # --- Resume from a previous, interrupted run if a resume file exists ---
    import os
    if os.path.exists(args.resume_path):
        print(f"Found {args.resume_path} -- resuming instead of starting over.")
        resume_state = torch.load(args.resume_path, map_location=device)

        # The cosine LR schedule is only mathematically valid over a FIXED
        # total length -- it's a single decay curve from lr down to ~0 over
        # exactly total_epochs steps. If you resumed with a different
        # --epochs than the original run, the optimizer's restored step
        # count would land at the wrong point on a *different* curve, and
        # past the original curve's midpoint the cosine can swing back
        # upward instead of continuing to decay. So --epochs must match the
        # original run when resuming -- resume is for surviving a crash on
        # the run you already started, not for extending it. Start a fresh
        # run (optionally seeded from ik_model.pt's weights) if you want to
        # train longer than originally planned.
        if resume_state["total_epochs"] != args.epochs:
            raise ValueError(
                f"{args.resume_path} was started with --epochs "
                f"{resume_state['total_epochs']}, but this run passed "
                f"--epochs {args.epochs}. Resuming with a different epoch "
                f"count would corrupt the cosine LR schedule. Re-run with "
                f"--epochs {resume_state['total_epochs']} to continue this "
                f"job, or delete {args.resume_path} to start a fresh one."
            )
        total_epochs = resume_state["total_epochs"]

        model.load_state_dict(resume_state["model_state_dict"])
        optimizer.load_state_dict(resume_state["optimizer_state_dict"])
        scheduler.load_state_dict(resume_state["scheduler_state_dict"])
        start_epoch = resume_state["epoch"] + 1
        best_mean_cm = resume_state["best_mean_cm"]
        if start_epoch > total_epochs:
            print(f"Resume file is already at epoch {resume_state['epoch']}, which meets or "
                  f"exceeds --epochs {total_epochs}. Nothing to do (delete "
                  f"{args.resume_path} to start a fresh, longer run instead).")
            return
        print(f"Resuming from epoch {start_epoch}/{total_epochs} "
              f"(best so far: {best_mean_cm:.3f} cm)")
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        do_eval = (epoch % args.eval_every == 0) or (epoch == total_epochs)
        if do_eval:
            ...
            print(f"Epoch {epoch:4d}/{total_epochs}  train_MSE={train_loss:.6f}  "
                  f"lr={scheduler.get_last_lr()[0]:.2e}")

        # --- Save full resume state EVERY epoch, not just on eval epochs.
        # This is what makes a crash/sleep/shutdown recoverable: at worst
        # you lose the partial progress of whichever single epoch was
        # running when it died, never the whole run. ---
        torch.save({
            "model_state_dict": model.state_dict(),
            "optimizer_state_dict": optimizer.state_dict(),
            "scheduler_state_dict": scheduler.state_dict(),
            "epoch": epoch,
            "best_mean_cm": best_mean_cm,
            "total_epochs": total_epochs,
        }, args.resume_path)

    print(f"\nBest validation mean Cartesian error: {best_mean_cm:.3f} cm")
    print(f"Best checkpoint saved to: {args.out}")
    print(f"Training complete -- you can now delete {args.resume_path} if you don't "
          f"plan to extend training further.")
